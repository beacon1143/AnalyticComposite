...
class FiberComposite(composite.Composite):
    def compute_eff_elast(self):
        ...
        self.effective.el_props.E[0] = self.incl.el_props.get_E() * gam_fib + self.matr.el_props.get_E() * gam_matr + \
        ...
        self.effective.el_props.nu[0, 1] = gam_fib * self.incl.el_props.get_nu() + gam_matr * self.matr.el_props.get_nu() + \
        ...
        self.effective.el_props.nu[0, 0] = self.effective.el_props.nu[0, 1]
        ...
        # shear module, G_LT
        self.effective.el_props.G[0] = self.matr.el_props.get_G() * (
            (1.0 + gam_fib) * self.incl.el_props.get_G() + (1.0 - gam_matr) * self.matr.el_props.get_G()
        ) / (
            (1.0 - gam_fib) * self.incl.el_props.get_G() + (1.0 + gam_matr) * self.matr.el_props.get_G()
        )

        self.effective.el_props.G[1] = self.effective.el_props.G[0]

        # shear module, G_TT
        self.effective.el_props.G[2] = self.matr.el_props.get_G() * (1.0 + gam_fib / (
        ...
        # auxiliary variables
        coef = 4.0 * self.effective.el_props.nu[0, 0] ** 2 * self.effective.el_props.G[2] * k23
        frac = coef / self.effective.el_props.E[0]

        # Young's modulus across fibers, E_T
        self.effective.el_props.E[1] = 4.0 * self.effective.el_props.G[2] * k23 / (
            k23 + self.effective.el_props.G[2] + frac
        )

        self.effective.el_props.E[2] = self.effective.el_props.E[1]

        # Poisson's ratio, nu_TT
        self.effective.el_props.nu[1, 1] = (k23 - self.effective.el_props.G[2] - frac) / (
            k23 + self.effective.el_props.G[2] + frac
        )

        self.effective.el_props.nu[2, 1] = self.effective.el_props.nu[1, 1]

        # Poisson's ratio, nu_TL
        self.effective.el_props.nu[1, 0] = 4.0 * coef / (
            self.effective.el_props.E[0] * (k23 + self.effective.el_props.G[2]) + coef
        )

        self.effective.el_props.nu[2, 0] = self.effective.el_props.nu[1, 0]

        self.effective.el_props.compute_Cijkl_from_ortho()
